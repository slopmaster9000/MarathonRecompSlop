...
// This file deliberately lives inside that translation unit so it can use the
// renderer's private GuestSurface/back-buffer state without widening upstream
// interfaces for the experimental integration.
...
static void DLSSPrepareFrameResources()
{
    ...
    const bool outputChanged =
        ...
        g_dlssOutputTexture->getWidth() != g_dlssOutputWidth ||
        g_dlssOutputTexture->getHeight() != g_dlssOutputHeight;

    const bool motionChanged =
        g_dlssMotionTexture == nullptr ||
        g_dlssMotionTexture->getWidth() != g_dlssRenderWidth ||
        g_dlssMotionTexture->getHeight() != g_dlssRenderHeight;
...
static bool DLSSEvaluateRenderedFrame()
{
    ...
    // This initial POC supplies object motion as explicitly invalid. Streamline
    // then reconstructs camera motion from the real current/previous camera
    // matrices and depth. Moving-object velocity is the next fidelity step.
    ...
    // From this point onward the logical backbuffer is the full-resolution DLSS
    // output. The already-generated ImGui draw command therefore renders after
    // DLSS at output resolution, and the existing gamma pass samples this texture.
    ...
    // Streamline and the motion clear changed native command-list/framebuffer
    // state. The ImGui path explicitly rebinds its pipeline/descriptors/viewport;
    // force its framebuffer cache to be rebuilt as well.
